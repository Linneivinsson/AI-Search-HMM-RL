...
class PlayerController(SettingLoader, Communicator):
    ...
    def _pipes_ready(self):
        return getattr(self, "sender_pipe", None) is not None and getattr(self, "receiver_pipe", None) is not None
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def player_loop(self):
        ...
        self.init_actions()
        self.init_states()
        self.alpha = self._get_setting("alpha", 0)
        self.gamma = self._get_setting("gamma", 0)
        self.epsilon_initial = self._get_setting("epsilon_initial", 1)
        self.epsilon_final = self._get_setting("epsilon_final", 0.2)
        self.annealing_timesteps = self._get_setting("annealing_timesteps", 10000)
        self.threshold = self._get_setting("threshold", 1e-6)
        self.episode_max = self._get_setting("episode_max", self.episode_max)
    ...
    def q_learning(self):
        ...
        init_pos_tuple = self._get_setting("init_pos_diver", [0, 0])
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        if self.actions is None:
            self.init_actions()
        if self.ind2state is None or self.state2ind is None:
            self.init_states()
        if not hasattr(self, "allowed_moves"):
            self.allowed_movements()
