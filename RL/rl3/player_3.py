...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        action = int(np.random.choice(all_actions)) if np.random.rand() < epsilon else int(
            np.random.choice([a for a in all_actions if Q[state, a] == np.nanmax(Q[state, all_actions])]))
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.1
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def player_loop(self):
        ...
        self.init_actions()
        self.init_states()
        king = tuple(self.settings.pos_king) if hasattr(self.settings, "pos_king") and self.settings.pos_king is not None else (0, 0)
        jx = getattr(self.settings, "jelly_x", []) or []
        jy = getattr(self.settings, "jelly_y", []) or []
        jellies = set(zip(jx, jy))
        policy = self.shortest_path_policy(king, jellies)
        msg = {"policy": policy, "exploration": False}
        self.sender(msg)
        while True:
            self.receiver()

    def shortest_path_policy(self, king, jellies):
        size = self.space_subdivisions
        INF = (10 ** 9, 10 ** 9)
        dist = {king: (0, 0)}
        heap = [(0, 0, king)]
        while heap:
            steps, jelly_hits, u = heapq.heappop(heap)
            if (steps, jelly_hits) != dist.get(u):
                continue
            x, y = u
            preds = [((x + 1) % size, y), ((x - 1) % size, y)]
            if y > 0:
                preds.append((x, y - 1))
            if y < size - 1:
                preds.append((x, y + 1))
            add_jelly = 1 if u in jellies else 0
            for v in preds:
                cand = (steps + 1, jelly_hits + add_jelly)
            ...
            for y in range(size):
                ...
                for action in self.action_list:
                    ...
                    if action == "left":
                        ...
                        ny = y + 1 if y < size - 1 else y
                    elif action == "down":
                        ny = y - 1 if y > 0 else y
                    nxt = (nx, ny)
                    d = dist.get(nxt, INF)
                    cand = (d[0] + 1, d[1] + (1 if nxt in jellies else 0))
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                list_pos = [self.action_list.index(a) for a in self.allowed_moves[s_current]]
                ...
                target = R if end_episode else R + discount * np.nanmax(Q[s_next, :])
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        n = np.zeros((ns, na))
        for s in range(ns):
            allowed = set(self.allowed_moves[s])
            for a in range(na):
                if self.action_list[a] not in allowed:
                    n[s, a] = np.nan
        # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = int(np.random.choice([self.action_list.index(a) for a in possible_actions]))
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        if self.schedule_timesteps <= 0:
            return self.final_p
        t = min(max(0.0, float(t)), float(self.schedule_timesteps))
        frac = t / float(self.schedule_timesteps)
