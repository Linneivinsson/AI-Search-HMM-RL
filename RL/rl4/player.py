...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.random() < epsilon: # explore
            ...
            action = all_actions[np.nanargmax(Q[state, all_actions])]
        ...
        if np.random.random() < epsilon.value(current_total_steps): # explore
            action = np.random.choice(all_actions)
        else:
            action = all_actions[np.nanargmax(Q[state, all_actions])]
        # ADD YOUR CODE SNIPPET BETWEENEX  4.2
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, steps, epsilon_initial=1, epsilon_final=0.2)
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        e = self.initial_p+ (delta_epsilon * t/ self.schedule_timesteps)
